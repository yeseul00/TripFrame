...
- **logic/reverseEngine.ts** - Calculates departure time by subtracting duration steps from anchor time
- **logic/gapEngine.ts** - Detects missing transport between location-based events

### Key Data Types
- `TripEvent` - Any timeline item (flight, hotel, transport, activity)
- `Gap` - Detected issue between events with severity: `DANGER` (no transport), `WARNING` (<30min buffer), `OK`
...
9. **date-fns for dates** - Standard time library
...
3. P3: Booking alerts + Pass analysis

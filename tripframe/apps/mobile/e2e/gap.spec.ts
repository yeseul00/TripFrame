/**
 * E2E 테스트: SCR-002 — 이동 체크 탭 Gap 감지 (MoveCheckScreen)
 *
...
test.describe('SCR-002 이동 체크 탭 — Gap 감지', () => {
  test.beforeEach(async ({ page }) => {
    await selectMockTrip(page);
    await page.locator('text=이동 체크').click();
  });

  test('[SCR-002-01] "이동 체크" 탭 표시', async ({ page }) => {
  ...
  test('[TC-011] DANGER 배지가 1개 이상 표시됨', async ({ page }) => {
    const dangerCards = page.getByText('DANGER');
